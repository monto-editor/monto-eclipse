...
public class Service<A> {
  ...
  public Service(Source source, Product product, Language language,
      Function<ProductMessage, Optional<A>> parser) {
    this(String.format("%s %s %s", source, product, language), parser);
  }

  public Service(Source source, Product product, Language language, String serviceID,
      Function<ProductMessage, Optional<A>> parser) {
          ...
          while (running) {
            ...
            withLock(() -> {
              if ((state == Fetch.PENDING || state == Fetch.WAITING)
                  && message.map(msg -> msg.getId().upToDate(versionID)).orElse(false)) {
